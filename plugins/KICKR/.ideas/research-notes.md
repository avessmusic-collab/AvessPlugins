# KICKR — Reference Research Notes
...
**Implication for KICKR default (55 Hz, 4×, ~50 ms):** solid neutral techno/house punch. Keep default `pitchCurve` ~0.7 (toward exponential). Consider a subtle **two-stage** shape (very fast first 5–10 ms, then a slower settle) — several pro synths do this to get click + weight without a mid sweep.

---

## 2. Transient / click generator

**Reference architectures diverge here — and this is where KICKR differentiates:**
- **Sonic Academy Kick 2:** click = **sampled** layers (3 independent click layers, drag-drop samples, each with start/length/env/filter/pitch). Sub/body = clean sine + up to 7 addable harmonics. *KICKR spec forbids samples — so we synthesize the click instead.*
...
**Synthesized click recipe (algorithmic, per KICKR spec):** blend of
...
- **Automatic gain compensation is standard** — "ensures the output sounds as loud as the input", RMS/loudness matching for honest A/B. KICKR must do this per-algorithm so a `character` sweep doesn't jump in loudness.
- Morphing is done by **A→B→C→D interpolation between slots** (MWaveShaper X-Y pad; DYNA "elastic morphing"). For KICKR's single knob: place the 7 curves on a 1-D path and **crossfade adjacent pairs**.
...
**Recommendation for KICKR's *safety* limiter (goal = catch overs, not squash):**
...
| **Devious Machines Kick Ninja** | **synthesis-first**, no sample-library reliance | "low end holds its shape when pushed hard"; positioned against sample-based competitors — closest philosophical match to KICKR |
...
5. **Deep but fast** — macro/one-knob shaping on top of full editability. KICKR's Punch/Body/Crush/Tail macros cover this.
...
**KICKR's differentiation angle:** fully algorithmic (no samples at all, including clicks), morphing distortion on a single knob, constrained musical Randomize/Mutate, native low-latency analyzer.
