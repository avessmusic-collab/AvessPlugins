...
- **The bank:** the user drops WAV / AIFF / FLAC / CAF files onto the plugin window. Each is decoded and **copied** into a managed library folder — `~/Library/Audio/Presets/PluginFreedom/KICKR/Samples/` — which the plugin scans and watches. The header's browser shuttles through the bank (prev / next / list), same UX as presets. **No factory samples ship** — the bank is empty until the user fills it.
