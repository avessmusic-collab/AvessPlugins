...
/**
 * Transitionist WebView-based Plugin Editor (Stage 3 / Phase 5.1)
 *
 * Adapted from plugins/Transitionist/.ideas/mockups/v4-PluginEditor.h/.cpp.
 * This is the real WebView editor built from the finalized v4 UI mockup,
 * replacing the Stage 1/2 placeholder editor.
 *
 ...
 *
 * NOTE ON "sweep" (BIPOLAR PARAMETER):
 ...
 * work happens in APVTS parameter creation (Stage 1/Foundation, already
 * done - see PluginProcessor.cpp's createParameterLayout()) and in the
 * HTML/JS arc-fill + LCD sign formatting (already handled in index.html's
 * makeBipolarRenderer()). See plugins/Transitionist/.ideas/parameter-spec.md
 * for the exact NormalisableRange declaration used for "sweep".
 *
 * NOTE ON APVTS MEMBER NAME: this codebase's PluginProcessor exposes its
 * AudioProcessorValueTreeState as `processorRef.parameters` (NOT `.apvts`
 * as used in the generic v4-PluginEditor.h/.cpp templates) - all parameter
 * lookups below use `processorRef.parameters.getParameter(...)`.
 */
...
private:
     ...
     * Maps URLs to embedded binary data via EXPLICIT URL mapping
     * (juce8-critical-patterns.md Pattern #8).
     *
